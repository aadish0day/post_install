...
install_if_needed() {
    ...
    for pkg in "$@"; do
        if ! pacman -Qi "$pkg" &> /dev/null; then
            echo "Installing $pkg..."
            sudo pacman -S "$pkg" --noconfirm || { echo "Failed to install $pkg"; failures+=("$pkg"); continue; }
        else
...
packages=(
    neovim ranger ncdu mpv maven yt-dlp fzf git unzip nodejs gcc make git ripgrep fd unzip htop gettext libtool doxygen flameshot npm xclip ueberzug highlight atool mediainfo neofetch android-tools img2pdf zathura zathura-pdf-poppler zathura-ps zathura-djvu zathura-cb obs-studio picom nitrogen starship xss-lock qalculate-qt libreoffice-still brightnessctl qbittorrent bluez bluez-utils blueman
)
